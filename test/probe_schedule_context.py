...
RM._report_start_session = _forbidden
RM.auto_schedule_check = _forbidden
...
def main():
    ...
    # ---------- 2. 已啟用排程 ----------
    plans, ok = RM.fetch_enabled_plans(client, RM._SCHED_TPL_LIST, RM._SCHED_ITEM_LIST)
    print(f"\n--- 已啟用排程項目（enableFlag==1）--- ok={ok}  共 {len(plans)} 筆")
    today = datetime.now().date()
    for p in plans:
        print(f"  · {p['name']:<12} {p['start_txt']}~{p['end_txt']}  {p['cd']:<9} "
              f"{_when_text(p):<24} 今日適用={RM._plan_applies_on(p, today)}")
    if not plans:
        print("  （無啟用排程）→ 需先於 HMI 啟用一段排程才能安排實機驗證時段。")

    # ---------- 3. Schedule Context ----------
    now = datetime.now()
    ctx = (RM.compute_schedule_ctx(plans, now) if ok else
    ...
    print(f"  Dashboard 顯示   ={RM._schedule_ctx_text(ctx)}")
    ...
    print(f"  AUTO_HOLD_START_SEC={CFG.AUTO_HOLD_START_SEC}s  "
          ...
          f"Dashboard 刷新={RM.AUTO_REFRESH_SEC}s")
