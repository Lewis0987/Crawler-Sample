...
def run_check(r, client=None, source="auto", satisfy_hold=True):
    ...
    if satisfy_hold and isinstance(r, dict):
        _d = M._actual_direction(r)[0]
        if _d in ("charge", "discharge"):
            M._auto.update(direction=_d, since=M.time.monotonic() - 3600,
...
_bg_fn = _src_menu[_src_menu.index("def _report_bg_start("):
                   _src_menu.index("def _stop_and_finalize(")]
...
check("收尾走既有唯一入口 _report_finalize()",
      "_report_finalize(" in _src_menu[_src_menu.index("def _auto_finalize_from_loop("):
                                       _src_menu.index("def _report_bg_start(")])
...
_fin_fn = _src_menu[_src_menu.index("def _report_finalize("):
                    _src_menu.index("def _report_start_session(")]
...
_lastdir = M._last_active_direction
...
_sync_fn = _src_menu[_src_menu.index("def _sync_stop_pending("):
                     _src_menu.index("def _auto_finalize_from_loop(")]
...
reset_monitor()
M._report["session"] = FakeSession("S1")
M._auto["state"] = M.AUTO_RUNNING
M._sync_stop_pending(_HeldSess(12.0))
check("idle 累積中 → state 轉為 STOP_PENDING", M._auto["state"] == M.AUTO_STOP_PENDING)
M._sync_stop_pending(_HeldSess(0.0))
check("方向回充/放（held=0）→ state 回 RUNNING", M._auto["state"] == M.AUTO_RUNNING)
M._report["stopping"] = True
M._auto["state"] = M.AUTO_RUNNING
M._sync_stop_pending(_HeldSess(99.0))
check("收尾進行中（stopping=True）→ 不覆寫狀態", M._auto["state"] == M.AUTO_RUNNING)
M._report["stopping"] = False

print("\n[Phase 3.3] Dashboard 顯示 idle 倒數")
reset_monitor()
M._report["session"] = _HeldSess(42.0)
_line = M._monitor_status_line(reading(direction="idle"), "auto")
check(f"狀態行含 idle=42s/{CFG.AUTO_HOLD_STOP_SEC}s（{_line[-40:]}）",
      f"idle=42s/{CFG.AUTO_HOLD_STOP_SEC}s" in _line)
M._report["session"] = None
_line2 = M._monitor_status_line(reading(direction="idle"), "auto")
...
check("menu 不再自行宣告 SCHED_CD_CODE",
      not re.search(r"^SCHED_CD_CODE\s*=", _msrc_d1, re.M))
check("menu 不再自行定義 _as_int_code()",
...
check("menu 使用 CDR_CFG.SCHED_CD_CODE / CDR_CFG.as_int",
      "CDR_CFG.SCHED_CD_CODE" in _msrc_d1 and "CDR_CFG.as_int" in _msrc_d1)
...
check("命名整理：_auto_enabled_plan_state()（三態，非 bool）",
      callable(getattr(M, "_auto_enabled_plan_state", None))
      and not hasattr(M, "_auto_template_status"))

print("\n[Phase 3.6] atexit 保底：行程結束時把 Session 標記為 paused")
check("模組層已註冊 atexit", "atexit.register(_atexit_guard)" in _msrc_d1)
_guard_src = _msrc_d1[_msrc_d1.index("def _atexit_guard("):
                      _msrc_d1.index("atexit.register(_atexit_guard)")]
...
reset_monitor()
check("無 Session → atexit 不動作、不拋例外", M._atexit_guard() is None)
_ms = _MarkSess()
M._report.update(session=_ms, thread=None, stop=None, finalized=False)
M._atexit_guard()
check(f"有 Session → 呼叫 mark_paused 一次（{_ms.marked}）", _ms.marked == 1)
M._report.update(session=_ms, finalized=True)
M._atexit_guard()
check("已 finalized → 不重複標記（idempotent）", _ms.marked == 1)
reset_monitor()
M._report["session"] = FakeSession("NO_MARK")     # 測試替身沒有 mark_paused
check("Session 無 mark_paused 方法 → 不拋例外", M._atexit_guard() is None)
reset_monitor()
...
_g = M._orphan_gap_seconds(_mk_state(299))
check(f"斷線 299s → gap≈299（{_g:.0f}）且未逾門檻", 295 <= _g <= 305 and _g <= 300)
_g = M._orphan_gap_seconds(_mk_state(600))
...
check("last_sample_time 缺失 → 回 None（保守，不誤判）",
      M._orphan_gap_seconds(_d_no) is None)
...
check("last_sample_time 格式非預期 → 回 None（不拋例外）",
      M._orphan_gap_seconds(_d_bad) is None)
check("無 session_state.json → 回 None",
      M._orphan_gap_seconds(_tf.mkdtemp(prefix="p36_")) is None)
...
_res_fn = _func_src(_msrc_d1, "report_resume_on_launch")
...
check("report_pause() 未加入 idle→finalize 判斷",
      "_report_finalize" not in _func_src(_msrc_d1, "report_pause"))

print("\n[Phase 3.5] Schedule Context —— 常數與預設值")
check(f"AUTO_NEXT_PLAN_GAP_SEC = 0（Phase 3.5 初始值＝停用，不改變 Auto Stop 行為）",
...
def _plan(name, ptype, days_or_date, s, e, cd="charge"):
    return {"name": name, "plan_type": ptype,
            ...
            "start_min": M._hhmm_to_min(s), "end_min": M._hhmm_to_min(e),
...
for _t, _exp in (("16:00", False), ("17:00", True), ("17:30", True),
    ...
    _c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, int(_t[:2]), int(_t[3:])), grace_sec=0)
    check(f"每週排程 {_t} → in_window={_c['in_window']}（預期 {_exp}）",
          _c["in_window"] is _exp)
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, 17, 30), grace_sec=0)
...
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 8, 17, 30), grace_sec=0)
check("執行日不符（週六）→ in_window=False", _c["in_window"] is False)
# 停用模板不會進入 plans（由 fetch_enabled_plans 過濾）→ 空清單
_c = M.compute_schedule_ctx([], _DT(2026, 8, 7, 17, 30), grace_sec=0)
...
for _d, _t, _exp in ((7, "21:00", False), (7, "23:00", True),
    ...
    _c = M.compute_schedule_ctx(_p_night, _DT(2026, 8, _d, int(_t[:2]), int(_t[3:])),
                                grace_sec=0)
...
for _d, _exp in ((6, False), (7, True), (8, False)):
    _c = M.compute_schedule_ctx(_p_date, _DT(2026, 8, _d, 10, 30), grace_sec=0)
    check(f"08-{_d:02d} 10:30 → in_window={_c['in_window']}（預期 {_exp}）",
          _c["in_window"] is _exp)

print("\n[Phase 3.5] 邊界與寬限（AUTO_WINDOW_GRACE_SEC）")
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, 16, 59), grace_sec=0)
check("無寬限：窗口前 1 分鐘 → out", _c["in_window"] is False)
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, 16, 59), grace_sec=120)
check("寬限 120s：窗口前 1 分鐘 → in（吸收設備/PC 時鐘落差）", _c["in_window"] is True)
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, 17, 51), grace_sec=120)
check("寬限 120s：窗口後 1 分鐘 → in", _c["in_window"] is True)
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, 17, 53), grace_sec=120)
check("寬限 120s：窗口後 3 分鐘 → out", _c["in_window"] is False)

print("\n[Phase 3.5] next_plan_in_sec")
_c = M.compute_schedule_ctx(_p_week, _DT(2026, 8, 7, 16, 0), grace_sec=0)
...
_c = M.compute_schedule_ctx(_p_adj, _DT(2026, 8, 7, 17, 51), grace_sec=0)
...
_ctx_soon = {"in_window": False, "current_plan": None,
             "next_plan_in_sec": 60.0, "source": "ok"}
try:
    CFG.AUTO_NEXT_PLAN_GAP_SEC = 0                      # 停用（Phase 3.5 初始值）
    ...
    CFG.AUTO_NEXT_PLAN_GAP_SEC = 300                    # 啟用（未來驗證用）
    check("gap=300 + 下段 60s 後開始 → 阻擋收尾（維持同一份報告）",
          _sess_idle(70).should_auto_end(schedule_ctx=_ctx_soon) == (False, None))
...
class _SchedCli:
    ...
    def get(self, path, **_k):
        ...
        if path == M._SCHED_TPL_LIST:
            return self.tpls
        for tid, v in self.items.items():
            if path == M._SCHED_ITEM_LIST.format(tid=tid):
                return v
        return []
...
_pl, _ok = M.fetch_enabled_plans(_SchedCli(_T_EN, _I_OK), M._SCHED_TPL_LIST,
                                 M._SCHED_ITEM_LIST)
check(f"啟用模板 → 取得 {len(_pl)} 筆項目、ok={_ok}", _ok is True and len(_pl) == 1)
check("方向正規化為 charge", _pl[0]["cd"] == "charge")
_pl2, _ = M.fetch_enabled_plans(
    _SchedCli(_T_EN, {"1": [{"startTime": "17:00", "endTime": "17:50",
                             "chargeOrDischarge": "2"}]}),
    M._SCHED_TPL_LIST, M._SCHED_ITEM_LIST)
check("chargeOrDischarge 為字串 \"2\" → 正規化為 discharge", _pl2[0]["cd"] == "discharge")
_T_DIS = [dict(_T_EN[0], enableFlag=2)]
_pl3, _ = M.fetch_enabled_plans(_SchedCli(_T_DIS, _I_OK), M._SCHED_TPL_LIST,
                                M._SCHED_ITEM_LIST)
check("停用模板（enableFlag=2）不列入", _pl3 == [])
_pl4, _ok4 = M.fetch_enabled_plans(_SchedCli(_T_EN, _I_OK, fail=True),
                                   M._SCHED_TPL_LIST, M._SCHED_ITEM_LIST)
check("API 失敗 → ok=False", _ok4 is False)
_pl5, _ok5 = M.fetch_enabled_plans(_SchedCli({"bad": 1}, {}), M._SCHED_TPL_LIST,
                                   M._SCHED_ITEM_LIST)
check("模板清單格式非預期 → ok=False", _ok5 is False)
_pl6, _ = M.fetch_enabled_plans(
...
check("_hhmm_to_min 邊界：'00:00'→0、'23:59'→1439、'24:00'→None、''→None",
      M._hhmm_to_min("00:00") == 0 and M._hhmm_to_min("23:59") == 1439
      and M._hhmm_to_min("24:00") is None and M._hhmm_to_min("") is None)

print("\n[Phase 3.5] 快取：60 秒內只查一次；idle=0 時完全不查")
reset_monitor()
_cli = _SchedCli(_T_EN, _I_OK)
_ctx1 = M._schedule_window_ctx(_cli)
_n1 = _cli.calls
_ctx2 = M._schedule_window_ctx(_cli)
check(f"快取有效期內第二次呼叫不再查 API（{_n1} → {_cli.calls}）", _cli.calls == _n1)
check("兩次回傳同一份 ctx", _ctx1 is _ctx2)
M._auto["sched_ctx_at"] = M.time.monotonic() - CFG.AUTO_WINDOW_CACHE_SEC - 1
M._schedule_window_ctx(_cli)
check(f"快取過期後重新查詢（{_n1} → {_cli.calls}）", _cli.calls > _n1)
reset_monitor()
check("client=None → source=unknown（不拋例外）",
      M._schedule_window_ctx(None)["source"] == "unknown")
reset_monitor()
_bg = _src_menu[_src_menu.index("def _report_bg_start("):
                _src_menu.index("def _stop_and_finalize(")]
...
check("API 失敗 → window=unknown",
      M._schedule_ctx_text({"source": "unknown"}) == " window=unknown")
check("非 dict → 空字串（不拋例外）", M._schedule_ctx_text(None) == "")
...
for _c in ("AUTO_HOLD_START_SEC", "AUTO_HOLD_STOP_SEC", "AUTO_COOLDOWN_SEC"):
    check(f"menu 未自行宣告 {_c}（避免與 config 生效值矛盾）",
...
check("狀態機含 START_HOLD / COOLDOWN",
      M.AUTO_START_HOLD == "START_HOLD" and M.AUTO_COOLDOWN == "COOLDOWN")
...
def _hold_run(direction="charge", held=None, **over):
    ...
    M._auto_track_direction(direction)                    # 首次觀測 → held=0
    if held is not None:                                  # 以 monotonic 回推，非靠呼叫次數
        M._auto["since"] = M.time.monotonic() - held
...
check(f"狀態為 START_HOLD（{M._auto['state']}）", M._auto["state"] == M.AUTO_START_HOLD)
...
reset_monitor(auto_report=True)
M._auto_track_direction("charge")
M._auto["since"] = M.time.monotonic() - 1.0
_created = 0
for _i in range(10):
...
for _label, _kw, _expect in (
    ...
    reset_monitor(auto_report=True)
    M._auto_track_direction("charge")
    M._auto["since"] = M.time.monotonic() - 5.0
    M._auto["state"] = M.AUTO_START_HOLD                  # 先處於累積中
    _a, _w, _c = run_check(reading(**_kw),
                           client=FakeClient(templates=[{"tempId": "1", "enableFlag": 1}]))
    check(f"START_HOLD →{_label}→ 不建立（{_w}）",
          _a == "skipped" and _w == _expect and not _c)
    check(f"　　　　　　　→ 狀態回 IDLE（{M._auto['state']}）",
          M._auto["state"] == M.AUTO_IDLE)
# 已存在 Session
reset_monitor(auto_report=True)
M._auto["state"] = M.AUTO_START_HOLD
M._report["session"] = FakeSession("EXIST")
_a, _w, _c = run_check(reading(direction="charge"))
check(f"START_HOLD →已存在 Session→ 不建立（{_w}）",
      _a == "skipped" and _w == "session_exists" and not _c)
check(f"　　　　　　　→ 狀態轉 RUNNING（{M._auto['state']}）",
      M._auto["state"] == M.AUTO_RUNNING)

print("\n[Phase 3.4] COOLDOWN：期間一律不得建立，不論條件如何變化")
reset_monitor(auto_report=True)
_cd_sec = M._auto_start_cooldown()
check(f"_auto_start_cooldown() 設定 {_cd_sec}s 冷卻", _cd_sec == CFG.AUTO_COOLDOWN_SEC)
check(f"cooldown_until 為 monotonic 值（{type(M._auto['cooldown_until']).__name__}）",
      isinstance(M._auto["cooldown_until"], float))
check(f"剩餘秒數約 {CFG.AUTO_COOLDOWN_SEC}s",
      CFG.AUTO_COOLDOWN_SEC - 1 <= M._auto_cooldown_remaining() <= CFG.AUTO_COOLDOWN_SEC)
for _label, _kw in (("charge", dict(direction="charge")),
    ...
    M._auto_track_direction(_kw.get("direction", "charge"))
    M._auto["since"] = M.time.monotonic() - 999          # 即使 hold 早已達標
    _a, _w, _c = run_check(reading(**_kw),
                           client=FakeClient(templates=[{"tempId": "1", "enableFlag": 1}]))
    check(f"冷卻期間 {_label} → 不建立（{_w}）",
          _a == "skipped" and _w.startswith("cooldown=") and not _c)
check(f"冷卻期間狀態為 COOLDOWN（{M._auto['state']}）", M._auto["state"] == M.AUTO_COOLDOWN)
# 冷卻期滿 → 立即可重新建立
M._auto["cooldown_until"] = M.time.monotonic() - 0.01
M._auto_track_direction("charge")
M._auto["since"] = M.time.monotonic() - 30
_a, _w, _c = run_check(reading(direction="charge"),
                       client=FakeClient(templates=[{"tempId": "1", "enableFlag": 1}]))
check(f"冷卻期滿 → 立即可重新建立（{_a}）", _a == "started" and len(_c) == 1)
check("冷卻期滿後 cooldown_until 已清除", M._auto["cooldown_until"] is None)
check("冷卻不影響 Auto Stop（should_auto_end 未被 cooldown 干預）",
      "cooldown" not in _msrc[_msrc.index("def _report_bg_start("):
                              _msrc.index("def _stop_and_finalize(")])

print("\n[Phase 3.4] Auto Stop → Cooldown 可連續循環兩輪")
reset_monitor(auto_report=True)
_cycle_ok = True
for _round in (1, 2):
    M._auto.update(cooldown_until=None, state=M.AUTO_IDLE)
    M._auto_track_direction("charge")
    M._auto["since"] = M.time.monotonic() - 30
    _a, _w, _c = run_check(reading(direction="charge"),
                           client=FakeClient(templates=[{"tempId": "1", "enableFlag": 1}]))
    _cycle_ok = _cycle_ok and _a == "started" and len(_c) == 1
    M._report["session"] = None                          # 模擬 auto_stop 收尾完成
    M._auto_start_cooldown()                             # 收尾後進入冷卻
...
check("兩輪循環後狀態仍為 COOLDOWN（未殘留錯誤狀態）",
      M._auto["state"] == M.AUTO_COOLDOWN)

print("\n[Phase 3.4] 顯示：state 名稱 + 對應倒數（三者擇一）")
reset_monitor()
M._auto.update(state=M.AUTO_START_HOLD, held_sec=8.0)
_l = M._monitor_status_line(reading(direction="charge"), "auto")
check(f"START_HOLD 顯示 state 與 hold=8s/15s（{_l[-46:]}）",
      "state=START_HOLD" in _l and f"hold=8s/{CFG.AUTO_HOLD_START_SEC}s" in _l)
M._auto.update(state=M.AUTO_COOLDOWN, cooldown_until=M.time.monotonic() + 42)
_l = M._monitor_status_line(reading(direction="idle"), "auto")
check(f"COOLDOWN 顯示 state 與 cooldown=42s/60s（{_l[-46:]}）",
      "state=COOLDOWN" in _l and f"cooldown=42s/{CFG.AUTO_COOLDOWN_SEC}s" in _l)
M._auto.update(state=M.AUTO_RUNNING, cooldown_until=None)
M._report["session"] = _HeldSess(30.0)
_l = M._monitor_status_line(reading(direction="idle"), "auto")
...
check("_auto_clear_notes() 一併清除 start/cooldown 去重 key",
      all(k in _msrc[_msrc.index("def _auto_clear_notes("):
                     _msrc.index("def _auto_cooldown_remaining(")]
          for k in ("last_start_note", "last_cooldown_note")))
check("cooldown_until 存於 _auto（Phase 4 可直接沿用，非 Dashboard 專用變數）",
      "cooldown_until" in M._auto)
...
_l_chg = M._monitor_status_line(_r_chg, "auto")
...
_l_stop = M._monitor_status_line(_r_stop, "auto")
...
check("待機時含 stby=True",
      " stby=True" in M._monitor_status_line(_r_stby, "auto"))
_r_none = reading(direction="idle")
_r_none.update(pcs_running_flag=None, pcs_standby_flag=None)
_l_none = M._monitor_status_line(_r_none, "auto")      # 欄位缺失不得拋例外
...
check("reading 完全缺少該兩欄位時仍可產生狀態行（不拋例外）",
      " run=None" in M._monitor_status_line(_r_miss, "auto"))
_ml_fn = _src_menu[_src_menu.index("def _monitor_status_line("):
                   _src_menu.index("def _monitor_footer(")]
...
check("_auto 具備獨立的 last_stop_note key", "last_stop_note" in M._auto)
# 模擬實際節奏：背景取樣器每輪（5s）印停止提示，Dashboard 每 3 輪（15s）印一般提示
_n_stop = _n_dash = 0
for _cy in range(1, 7):
    if M._auto_note(_STOP_MSG, key="last_stop_note"):
        _n_stop += 1
    if _cy % 3 == 0 and M._auto_note(_DASH_MSG):
        _n_dash += 1
...
check("兩者互不影響：last_note 與 last_stop_note 各自保存",
      M._auto["last_note"] == _DASH_MSG and M._auto["last_stop_note"] == _STOP_MSG)
# 一般提示內容改變時仍應重印（去重不能過度抑制）
check("一般提示內容改變 → 仍會重印", M._auto_note("[AUTO] 換一則訊息") is True)
check("停止提示不受一般提示影響（仍被抑制）",
      M._auto_note(_STOP_MSG, key="last_stop_note") is False)

print("\n[Phase 3.3] _auto_clear_notes()：Session reset 時統一清除所有去重 key")
M._auto.update(last_note="a", last_status="b", last_stop_note="c")
M._auto_clear_notes()
check("三個 key 一次全部清除",
      M._auto["last_note"] is None and M._auto["last_status"] is None
      and M._auto["last_stop_note"] is None)
_menu_src2 = open(os.path.join(HERE, "device_control_menu.py"), encoding="utf-8").read()
...
reset_monitor()
M._auto["last_stop_note"] = _STOP_MSG            # 上一份 Session 遺留
_noop = _NoopSess()
M._report_bg_start(_noop)                        # ← 正式路徑：新 Session 起始
try:
    check("新 Session 起始後 last_stop_note 已清除（不沿用上一份）",
          M._auto["last_stop_note"] is None)
    check("新 Session 可再次正常印出停止提示",
          M._auto_note(_STOP_MSG, key="last_stop_note") is True)
finally:
    if M._report["stop"] is not None:
        M._report["stop"].set()
    if M._report["thread"] is not None:
        M._report["thread"].join(timeout=5)
    reset_monitor()
check("測試用背景執行緒已收尾", M._report["thread"] is None)
...
try:
    ...
    M._report_bg_start(_sess)                      # ← 正式背景取樣執行緒
    M._auto["state"] = M.AUTO_RUNNING
    _t.sleep(1.0)                                  # 充電階段
    _n_charge = len(_sess.samples)
    check(f"充電階段持續取樣（{_n_charge} 筆）且未收尾",
          _n_charge >= 2 and M._report["session"] is not None)

    _phase["dir"] = "idle"                         # ← 排程結束，設備回 idle
    _deadline = _t.monotonic() + 15
    while M._report["session"] is not None and _t.monotonic() < _deadline:
        _t.sleep(0.1)
    check("idle 持續達門檻 → 背景自動收尾（_report[\"session\"] 已清空）",
          M._report["session"] is None)
    check(f"背景執行緒已結束（{M._report['thread']}）", M._report["thread"] is None)
    # Phase 3.4 起：auto_stop 收尾後進入 COOLDOWN（非直接 IDLE），冷卻期滿才回 IDLE
    check(f"狀態機進入 COOLDOWN（{M._auto['state']}）", M._auto["state"] == M.AUTO_COOLDOWN)
    check(f"冷卻已啟動（剩餘 {M._auto_cooldown_remaining():.0f}s）",
          M._auto_cooldown_remaining() > 0)
...
_fin_fn = _code_only(_func_src(_msrc_37, "_report_finalize"))
