...
# 🔴 Phase D.4-E 以**官方年度時間電價日曆**核對後的結論：
#    本 fixture 的 8 個日期**全部正確**，但**並不完整** ——
#    缺少官方 2026 年度日曆同樣標示為離峰日的春節（2/16~2/21）、
#    端午(6/19)、中秋(9/25)（皆需年度資料，測試不便），
#    以及所有週日（由日型規則涵蓋，不列於年度清單）。
#    ⚠️ D.4-D 曾判定教師節(9/28)／光復(10/25)／行憲(12/25)「不在官方清單」，
#       該結論已由官方年度時間電價日曆的直接證據**撤銷**：這三項確為離峰日。
#    本 fixture 的用途只是「餵一組固定日期給時段規則做邊界測試」，
#    **不代表**任何 production 語意。正式規則見 taipower_offpeak_rules.py，
#    正式年度資料見 annual_off_peak_calendar.PRODUCTION_ANNUAL_LISTS（目前為空）。
#    ⚠️ 刻意**不**修改本 fixture 的內容 —— 改它只會讓既有邊界測試失去意義，
#       而 production 規則本來就不從這裡取得。
FIXTURE_2026 = {
    date(2026, 1, 1),    # 開國紀念日
